...
//            newsq.sql = "insert into ";  add shop to orders - need build customer table first
        ...
        {
            sqlcon newsql = new sqlcon();
            newsql.sql = "select food.id,food.name,food.meal,food.kind,food.price,food.score,food.rest_name,food.time_prepare,list.list_name,list.list_date,food.score,feed_back.feed_back from rest_manager.dbo.list inner join  rest_manager.dbo.list_match on(list.list_name=list_match.list_name and list.list_meal=list_match.list_meal and list.list_date=list_match.list_date) inner join dbo.food on (list_match.food_name=food.name and list_match.food_meal=food.meal) left join feed_back on (food.id=feed_back.food_id)  " + "where list.list_date='" + dateTimePicker1.Value.ToString("yyyy/MM/dd") + "' and list_match.food_meal='" + comboBox1.Text + "' and list.list_name='" + comboBox2.Text + "'";
        ...
        {
            sqlcon newsql = new sqlcon();
            newsql.sql = "select food.id,food.name,food.meal,food.kind,food.price,food.score,food.rest_name,food.time_prepare,list.list_name,list.list_date,feed_back.feed_back from rest_manager.dbo.list inner join  rest_manager.dbo.list_match on(list.list_name=list_match.list_name and list.list_meal=list_match.list_meal and list.list_date=list_match.list_date) inner join dbo.food on (list_match.food_name=food.name and list_match.food_meal=food.meal)   left join feed_back on (food.id=feed_back.food_id)  " + "where list.list_date='" + dateTimePicker1.Value.ToString("yyyy/MM/dd") + "' and list_match.food_meal='" + comboBox1.Text + "' and list.list_name='" + comboBox2.Text + "' and food.rest_name='"+comboBox3.Text+"'";
            newsql.setdata_adaptor();
